...
- `zero_matches_policy` (`'relegate'` | `'double_relegate'` | `'remove'`, default `'relegate'`) governs what happens to a player who plays zero matches in their division during a round, set at tournament creation and editable later in "Manage tournament settings" - not cascaded onto `leagues` (same pattern as `num_promoted`/`num_relegated`), read directly off the `tournaments` row by `generateNextRound()`. Applied via `computePromotionMovesWithNoShows()` (`src/lib/promotion.ts`), which forces every zero-match player into one of the division's existing `num_relegated` slots regardless of where the normal standings sort would otherwise place them (a 0-match player isn't guaranteed to sort last under every tiebreaker), and never eligible for promotion - `'relegate'` sends them down one division same as any other relegated player, `'double_relegate'` sends them down two (skipping the division directly below), `'remove'` drops them from the tournament entirely (not carried into the next round's `league_players`/`league_player_drafts`). Every zero-match player is handled this way in full, however many there are in a division - it's not capped at `num_relegated`. Whenever this leaves a division short of what it would otherwise naturally have (a skipped division missing an expected arrival, a division that lost more players than its usual promotion/relegation turnover, or a 'remove' vacancy), the gap is backfilled by promoting extra players - the next-best after the normal promotion cutoff - up from the division below, cascading down through as many divisions as it takes. For `'relegate'`/`'double_relegate'` this always fully resolves (nobody actually leaves the tournament, so every division ends up exactly at the size the tournament's own `num_promoted`/`num_relegated` configuration would naturally produce anyway); for `'remove'` the tournament's population genuinely shrinks, so whichever division runs out of players below it to pull from (typically the bottom one) permanently absorbs that loss. Single-format tournaments have no next round, so this setting is unused for them.
